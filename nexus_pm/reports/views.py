...
from inventory.models import Alert, InventoryAdjustment, Rental
...
class StatisticsReportView(View):
    def get(self, request):
        ...
        for i in range(11, -1, -1):
            month = (now.replace(day=1) - timezone.timedelta(days=30 * i)).replace(
                day=1
            )
            months.append(month)
        ...
        stock_in_by_month = [
            StockEntry.objects.filter(
                entry_type="in", timestamp__year=m.year, timestamp__month=m.month
        ...
        stock_out_by_month = [
            StockEntry.objects.filter(
                entry_type="out", timestamp__year=m.year, timestamp__month=m.month
            ).aggregate(total=Sum("quantity"))["total"]
            or 0
            for m in months
        ]
        ...
        rental_status_breakdown = list(
            Rental.objects.values("status").annotate(count=Count("id"))
        )
        ...
        rental_product_breakdown = list(
            Rental.objects.values("product__name")
        ...
        recent_transactions = []
        for entry in StockEntry.objects.select_related(
            "product", "created_by"
        ...
        for adj in InventoryAdjustment.objects.select_related(
            "product", "created_by"
        ).order_by("-timestamp")[:40]:
            recent_transactions.append(
                {
        ...
        for req in ProcurementRequest.objects.select_related("requester").order_by(
            "-created_at"
        )[:40]:
            recent_transactions.append(
                {
        ...
        recent_transactions = sorted(
            recent_transactions, key=lambda x: x["timestamp"], reverse=True
        )[:50]
        ...
        context = {
            ...
            "pending_procurement_requests": ProcurementRequest.objects.filter(
                status="pending"
            ).count(),
...
def statistics_report_export(request, format):
    ...
    stock_entries_df = pd.DataFrame(
        list(
            StockEntry.objects.select_related("product", "created_by").values(
                "id",
                "entry_type",
                "quantity",
                "location_from",
    ...
    inventory_adjustments_df = pd.DataFrame(
        list(
            InventoryAdjustment.objects.select_related("product", "created_by").values(
                "id",
                "adjustment_type",
                "quantity",
                "reason",
    ...
    rentals_df = pd.DataFrame(
        list(
            Rental.objects.select_related("product", "created_by").values(
                "id",
                "product__name",
                "quantity",
                "rented_to",
    ...
    alerts_df = pd.DataFrame(
        list(
            Alert.objects.select_related("product").values(
                "id",
                "product__name",
                "alert_type",
                "status",
    ...
    procurement_df = pd.DataFrame(
        list(
            ProcurementRequest.objects.select_related("requester", "decided_by").values(
                "id",
                "product_name",
                "requested_quantity",
                "current_stock",
            ...
            [
                {
                    ...
                    "Total Stock In": StockEntry.objects.filter(
                        entry_type="in"
                    ).aggregate(total=Sum("quantity"))["total"]
                    or 0,
                    "Total Stock Out": StockEntry.objects.filter(
                        entry_type="out"
                    ).aggregate(total=Sum("quantity"))["total"]
                    or 0,
                    "Current Stock": (
                        StockEntry.objects.filter(entry_type="in").aggregate(
                            total=Sum("quantity")
                        )["total"]
                        or 0
                    )
    ...
    if format == "excel":
        ...
        try:
            ...
            return HttpResponse(
                "PDF export requires xhtml2pdf. Please install it.", status=500
            )
