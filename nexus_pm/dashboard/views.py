...
class DashboardPageView(View):
    def get(self, request):
        ...
        total_stock_entries = (
            StockEntry.objects.aggregate(total=models.Sum("quantity"))["total"] or 0
        )
        increase_adj = (
            InventoryAdjustment.objects.filter(adjustment_type="increase").aggregate(
                total=models.Sum("quantity")
            )["total"]
            or 0
        )
        decrease_adj = (
            InventoryAdjustment.objects.filter(adjustment_type="decrease").aggregate(
                total=models.Sum("quantity")
            )["total"]
            or 0
        )
        total_stock_entries + increase_adj - decrease_adj
