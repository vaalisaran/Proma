...
def user_create(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            messages.success(
                request,
                f'✅ User "{user.username}" created. They can now login with the credentials you set.',
            )
...
def user_reset_password(request, pk):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            messages.success(
                request,
                f'✅ Password for "{reset_user.username}" has been reset successfully.',
            )
...
def settings_view(request):
    ...
    if request.method == "POST":
        ...
        if action == "update_profile":
            ...
            # We can store extra preferences in a session or just handle them in the UI for now 
            # if we don't want to add more fields to the model.
            # But let's assume we save what we have.
            user.save()
            ...
            messages.success(
                request,
                "Thank you! Your issue has been reported and will be reviewed by Project Managers.",
            )
            return redirect("accounts:settings")

        elif action == "update_system_settings" and request.user.is_admin:
            sys_settings.primary_color = request.POST.get(
                "primary_color", sys_settings.primary_color
            )
            sys_settings.font_size = request.POST.get(
                "font_size", sys_settings.font_size
            )
            sys_settings.default_pm_password = request.POST.get(
