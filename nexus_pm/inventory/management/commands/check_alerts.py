...
from inventory.models import Alert, QuantityLimit
        ...
        for limit in limits:
            ...
            if current_quantity <= limit.limit_quantity:
                ...
                existing_alert = Alert.objects.filter(
                    product=product, alert_type="limit_reached", status="active"
                ).first()
                ...
                if existing_alert:
                    ...
                    if existing_alert.current_quantity != current_quantity:
                        ...
                        alerts_updated += 1
                        self.stdout.write(
                            f"Updated alert for {product.name}: {current_quantity} <= {limit.limit_quantity}"
                        )
                else:
                    # Create new alert
                    ...
                    alerts_created += 1
                    self.stdout.write(
                        f"Created alert for {product.name}: {current_quantity} <= {limit.limit_quantity}"
                    )
            else:
                ...
                if existing_alert:
                    ...
                    existing_alert.save()
                    self.stdout.write(
                        f"Resolved alert for {product.name}: {current_quantity} > {limit.limit_quantity}"
                    )
