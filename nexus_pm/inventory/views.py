...
def _inventory_permission_redirect(request, access_field=None, manage_field=None):
    ...
    if (
        ...
        messages.error(
            request, "You do not have permission to manage actions on this page."
        )
        return redirect("dashboard-page")
    return None
...
def set_standard_limit(request):
    ...
    if request.method == "POST":
        ...
        if limit and limit.isdigit():
            ...
            messages.success(
                request,
                f"Standard limit set to {limit}. Applied to {products_without_limit.count()} products without specific limits.",
            )
        else:
    ...
    def post(self, request):
        ...
        if action == "create":
            ...
            if quantity > available_quantity:
                messages.error(
                    request,
                    f"Cannot rent {quantity} units of {product.name}. Only {available_quantity} available in stock.",
                )
...
def inventory_shortage_export_csv(request):
    ...
    if permission_redirect:
        return permission_redirect
    if not request.user.is_admin and not getattr(
        request.user, "can_manage_shortage_exports", True
    ):
