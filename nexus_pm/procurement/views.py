...
class ProcurementUploadView(View):
    def get(self, request):
        ...
        if request.user.is_admin:
            recent_requests = ProcurementRequest.objects.select_related(
                "requester", "product"
            )
        else:
    ...
    def post(self, request):
        ...
        if insufficient_count > 0:
            messages.warning(
                request,
                f"There are {insufficient_count} items with insufficient or zero stock. Please review the report below.",
            )
        elif results:
            messages.success(
                request, "Procurement request submitted and saved successfully."
            )
        if request.user.is_admin:
            recent_requests = ProcurementRequest.objects.select_related(
                "requester", "product"
    ...
    def handle_admin_decision(self, request, action):
        ...
        if not procurement_request.product:
            messages.error(
                request,
                "Cannot approve this request because product reference is missing.",
            )
            return redirect("procurement-upload")
        requested_qty = procurement_request.requested_quantity
        stock_in = (
            StockEntry.objects.filter(
                product=procurement_request.product, entry_type="in"
            ).aggregate(total=Sum("quantity"))["total"]
            or 0
        )
        stock_out = (
            StockEntry.objects.filter(
                product=procurement_request.product, entry_type="out"
            ).aggregate(total=Sum("quantity"))["total"]
            or 0
        )
        current_stock = stock_in - stock_out
        if requested_qty > current_stock:
            messages.error(
                request,
                f"Cannot approve request. Requested {requested_qty}, available {current_stock}.",
            )
            return redirect("procurement-upload")
        ...
        messages.success(
            request,
            f"Request approved and {requested_qty} stock deducted successfully.",
        )
        return redirect("procurement-upload")
...
class ProcurementRestockView(View):
    def post(self, request):
        ...
        requested_qty = request.POST.get("requested_qty") or request.POST.get(
            "restock_qty"
        )
        ...
        try:
            ...
            if requested_qty > current_stock:
                ...
                messages.warning(
                    request,
                    f"Alert sent: Requested {requested_qty}, but only {current_stock} in stock for {product.name}.",
                )
            else:
