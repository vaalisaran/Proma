...
from inventory.models import Alert, InventoryAdjustment, QuantityLimit
        ...
        try:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    datasheet_file = None
                    if "Datasheet Filename" in df.columns and not pd.isna(
                        row.get("Datasheet Filename")
                    ):
                    ...
                    product = Product.objects.create(
                        ...
                        shelf_number=str(row.get("Shelf Number", "")).strip()
                        if "Shelf Number" in df.columns
                        and not pd.isna(row.get("Shelf Number"))
                        else "",
...
class ProductDetailView(View):
    def get(self, request, pk):
        ...
        recent_stock_entries = StockEntry.objects.filter(product=product).order_by(
            "-timestamp"
        )[:10]

        # Get recent alerts
        recent_alerts = Alert.objects.filter(product=product).order_by("-created_at")[
            :5
        ]
        ...
        average_inventory = (
            ((stock_in + current_quantity) / 2)
            if (stock_in + current_quantity) > 0
            else 1
        )
        stock_turnover = (
            round(stock_out / average_inventory, 2) if average_inventory else 0
        )
        ...
        stock_stats = {
            ...
            "stock_in_count": StockEntry.objects.filter(
                product=product, entry_type="in"
            ).count(),
            "stock_out_count": StockEntry.objects.filter(
        ...
        rental_quantity = (
            product.rentals.filter(status="active").aggregate(total=Sum("quantity"))[
                "total"
            ]
            or 0
        )
            ...
            if adjustment_type and quantity:
                try:
                    ...
                    if quantity > 0:
                        ...
                        # Log the adjustment
                        AuditLog.log(
                            request.user, f"stock {adjustment_type}", stock_entry
                        )
                        ...
                        if adjustment_type == "in":
                            ...
                            messages.success(
                                request,
                                f"Successfully removed {quantity} units from {product.name}",
                            )
